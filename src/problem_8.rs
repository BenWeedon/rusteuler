...
//!
//! This solution relies mainly on the rust standard library. We create an
//! array where each element is one of the digits. We then call the
//! [`windows`](https://doc.rust-lang.org/std/primitive.slice.html#method.windows)
//! method on this array, which returns an iterator over windows of size 13.
//! Finally, for each window we get the product (being careful to use
//! [`u64`](https://doc.rust-lang.org/std/primitive.u64.html) to not overflow)
//! and call max on the full window iterator.
//!
//! ```
//! # rusteuler::framework::run_solution(8, || {
//! const DIGITS: [u8; 1000] = [
...
//! ];
//!
//! let solution: u64 = DIGITS
//!     .windows(13)
//!     .map(|s| s.iter().map(|i| *i as u64).product())
//!     .max()
//!     .unwrap();
//! assert_eq!(solution, 23514624000);
//! # Ok(())
//! # }).unwrap();
//! ```
//!
